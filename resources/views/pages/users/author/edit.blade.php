...
@section('title', 'Edit '.$author->user->name)
...
<div class="card shadow mb-4">
   <div class="card-header py-3">
      <div class="d-flex justify-content-between">
         <h6 class="m-0 font-weight-bold text-primary">Author - {{ $author->user->name }}</h6>
      </div>
   </div>
   <div class="card-body">
      <form action="{{ route('author.update', $author->user->id) }}" id="form" method="post" enctype="multipart/form-data">
         @csrf
         @method("PUT")
         <input type="hidden" name="user_id" value="{{ $author->user->id }}">
         <div class="row">
            <div class="col-6">
               <div class="form-group">
                  <label for="">Nama / Username</label>
                  <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ $author->user->name }}" placeholder="Nama / username">
                  @error('name')
            ...
            <div class="col-6">
               <div class="form-group">
                  ...
                  <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ $author->user->email }}" placeholder="Email">
                  @error('email')
                  <span class="invalid-feedback" role="alert">
                     <strong>{{ $message }}</strong>
                  </span>
            ...
            <div class="col-6">
               <div class="form-group">
                  ...
                  <select name="is_active" class="form-control @error('is_active') is-invalid @enderror" autocomplete="off">
                     <option value="1" {{ ($author->user->is_active == 1) ? 'selected' : '' }}>Aktif</option>
                     <option value="0" {{ ($author->user->is_active == 0) ? 'selected' : '' }}>Tidak Aktif</option>
                  </select>
                  @error('is_active')
                  <span class="invalid-feedback" role="alert">
                     <strong>{{ $message }}</strong>
